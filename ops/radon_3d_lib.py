...
from .ramp_filter import fbp_filter_op_custom

class ParallelBeamGeometry3DOp(object):
  def __init__(self, img_size, num_angles, op_snr, angle_max=np.pi/3,fact=2,filter_type='Ram-Lak'):
    ...
    self.filter_op = odl.tomo.analytic.filtered_back_projection.fbp_filter_op(self.op, filter_type=filter_type)
  ...
  def filter(self, y):
    return OperatorFunction.apply(self.filter_op, y)
  
  def get_filter(self, filter_type: str = None):
    """
    If None is passed, the filter type used to create the filter operator is returned.
    """
    if filter_type is None:
      ramp,four = fbp_filter_op_custom(self.op, filter_type=self.filter_type)
      return ramp,four
    else:
      ramp,four = fbp_filter_op_custom(self.op, filter_type=filter_type)
      return ramp,four
...
class ParallelBeamGeometry3DOpAngles_rectangular(ParallelBeamGeometry3DOp):
  def __init__(self, img_size, angles, op_snr, angle_max=np.pi/3, fact=2,filter_type='Ram-Lak'):
    # super().__init__(img_size, angles.shape[0], op_snr,angle_max, fact)
    ...
    self.detector_partition = odl.uniform_partition([-self.n1/(n), -self.n2/(n)], [self.n1/(n), self.n2/(n)], [fact*self.n1,fact*self.n2])
    #self.detector_partition = odl.tomo.parallel_beam_geometry(self.reco_space, self.num_angles).det_partition
    # TODO: change following axes to get rotation around another axis
    self.geometry = odl.tomo.Parallel3dAxisGeometry(self.angle_partition, self.detector_partition,
    axis=(1,0,0),det_axes_init=[(1, 0, 0), (0, 1, 0)],det_pos_init=(0,0,1))
